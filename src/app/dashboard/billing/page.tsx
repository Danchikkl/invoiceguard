...
const PLANS = [
  {
    key: 'free', name: 'Free', price: 0, desc: 'For getting started',
    features: [
      { text: 'Up to 3 invoices (lifetime)', ok: true },
      { text: 'PDF-style invoice detail', ok: true },
  ...
  {
    key: 'pro', name: 'Pro', price: 12, desc: 'For active freelancers',
    envVar: 'NEXT_PUBLIC_LS_VARIANT_PRO',
    features: [
      { text: 'Unlimited invoices', ok: true },
      { text: 'Automated follow-ups (24h/48h/72h/96h)', ok: true },
  ...
  {
    key: 'business', name: 'Business', price: 29, desc: 'For agencies & professionals',
    envVar: 'NEXT_PUBLIC_LS_VARIANT_BUSINESS',
    features: [
      { text: 'Everything in Pro', ok: true },
      { text: 'Remove InvoiceGuard branding', ok: true },
      { text: 'Your own email domain', ok: true },
...
export default function BillingPage() {
  ...
  async function handleUpgrade(planKey: string, envVar?: string) {
    const variantId = envVar ? (process.env as any)[envVar] : ''
    if (!variantId) {
      alert('Payment is not set up yet. Check back soon!')
      return
    }
    setLoading(planKey)
    try {
    ...
    <div className="p-8 max-w-4xl">
      <div className="mb-8">
        ...
        <p className="text-sm text-slate mt-0.5">
          Current plan: <span className="font-medium text-ink capitalize">{ready ? currentPlan : '…'}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {PLANS.map(plan => {
          ...
          return (
            <div key={plan.key} className={`rounded-xl p-5 border-2 transition-all ${
              isCurrent ? 'border-ink' : plan.highlight ? 'border-accent bg-accent-light/30' : 'border-gray-100 bg-paper'
            }`}>
              <div className="mb-4">
                <div className="flex items-center justify-between mb-1">
                  <h2 className="font-semibold text-ink">{plan.name}</h2>
                  {isCurrent && <span className="text-xs bg-ink text-paper px-2 py-0.5 rounded-full">Current</span>}
                  {plan.highlight && !isCurrent && <span className="text-xs bg-accent text-white px-2 py-0.5 rounded-full">Popular</span>}
                </div>
              ...
              {isCurrent ? (
                ...
                <div className="text-center text-xs text-slate py-2">Contact support to downgrade</div>
              ) : (plan as any).cta ? (
                <button
                  onClick={() => handleUpgrade(plan.key, (plan as any).envVar)}
                  disabled={loading === plan.key}
                  className={`w-full py-2.5 rounded text-sm font-medium transition-colors disabled:opacity-50 ${
                    plan.highlight ? 'bg-accent text-white hover:bg-blue-700' : 'bg-ink text-paper hover:bg-lead'
                  }`}
                >
                  {loading === plan.key ? 'Loading…' : (plan as any).cta}
                </button>
              ) : null}
            </div>
          )
