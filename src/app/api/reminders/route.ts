...
export async function GET(req: NextRequest) {
  ...
  for (const job of (due || [])) {
    try {
      ...
      const { data: profile } = await admin.from('profiles').select('full_name, email').eq('id', job.user_id).maybeSingle()
      const senderName = profile?.full_name || 'InvoiceGuard user'
      const senderEmail = profile?.email || ''

      await sendReminderEmail({
        to: recipient, senderName, senderEmail,
      ...
      await admin.from('activity_logs').insert({
        user_id: job.user_id, invoice_id: job.invoice_id,
        action: `${job.label} sent`, type: 'reminder_sent',
