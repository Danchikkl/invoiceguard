    ...
    try {
      ...
      if (clientEmail.trim()) {
        const { data: existing } = await supabase.from('clients').select('id')
          .eq('user_id', user.id).eq('client_email', clientEmail.trim().toLowerCase()).maybeSingle()
        if (existing) {
      ...
      if (validItems.length) {
        const rows = validItems.map((it, idx) => ({
          invoice_id: inv.id, user_id: user.id,
          description: it.description, quantity: it.quantity,
          unit_price: it.unit_price, amount: it.quantity * it.unit_price, position: idx,
        }))
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="card p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div><label className="label">Tax %</label><input className="input" type="number" min="0" max="100" step="0.1" value={taxRate} onChange={e => setTaxRate(Number(e.target.value))} /></div>
        </div>
      </div>
      ...
      <div className="card p-5">
        ...
        <div className="space-y-2 mb-3">
          ...
          {items.map((item, i) => (
            <div key={i} className="grid grid-cols-12 gap-2 items-center">
              <div className="col-span-6"><input className="input" placeholder="Design services" value={item.description} onChange={e => updateItem(i, 'description', e.target.value)} /></div>
              <div className="col-span-2"><input className="input text-right" type="number" min="0.01" step="0.01" value={item.quantity} onChange={e => updateItem(i, 'quantity', e.target.value)} /></div>
              <div className="col-span-2"><input className="input text-right" type="number" min="0" step="0.01" value={item.unit_price} onChange={e => updateItem(i, 'unit_price', e.target.value)} /></div>
              <div className="col-span-1 text-right text-sm font-medium">{formatMoney(item.quantity * item.unit_price, currency)}</div>
              <div className="col-span-1 flex justify-end">
                {items.length > 1 && <button type="button" onClick={() => removeItem(i)} className="text-slate hover:text-danger text-xl leading-none">×</button>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 space-y-1.5 max-w-xs ml-auto text-sm">
          <div className="flex justify-between text-slate"><span>Subtotal</span><span>{formatMoney(subtotal, currency)}</span></div>
          {taxRate > 0 && <div className="flex justify-between text-slate"><span>Tax ({taxRate}%)</span><span>{formatMoney(taxAmt, currency)}</span></div>}
          <div className="flex justify-between font-semibold text-ink pt-1 border-t border-gray-100"><span>Total</span><span>{formatMoney(total, currency)}</span></div>
        </div>
      </div>

      <div className="card p-5">
        <label className="label">Notes (optional)</label>
        <textarea className="input h-20 resize-none" placeholder="Payment terms, thank-you note…" value={notes} onChange={e => setNotes(e.target.value)} />
      </div>

      {error && <p className="text-sm text-danger">{error}</p>}

      <div className="flex items-center gap-3">
        <button type="submit" disabled={loading} className="btn-primary px-6">{loading ? 'Saving…' : 'Save invoice'}</button>
