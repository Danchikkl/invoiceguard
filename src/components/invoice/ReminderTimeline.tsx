          ...
          return (
            <div key={stage} className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {!sentDate && scheduledDate && isPending && <p className="text-xs text-slate">Scheduled {formatDate(scheduledDate)}</p>}
                {isCancelled && <p className="text-xs text-slate/50">Cancelled</p>}
                {isFailed && <p className="text-xs text-danger">Failed to send</p>}
                {!reminder && stage !== 0 && invoiceStatus !== 'draft' && (
                  <p className="text-xs text-slate/40">Not scheduled (Free plan)</p>
                )}
              </div>
            </div>
          )
