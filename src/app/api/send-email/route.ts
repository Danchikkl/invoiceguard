...
import { checkCanSendReminders } from '@/lib/paywall'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: inv, error: invErr } = await admin
      .from('invoices')
      .select('*, client:clients(client_name, client_email)')
      .eq('id', invoiceId)
      .eq('user_id', user.id)
      .maybeSingle()
    ...
    const { data: profile } = await admin.from('profiles').select('full_name, email, plan').eq('id', user.id).maybeSingle()
    ...
    // Проверяем может ли пользователь использовать авто-напоминания
    const reminderCheck = await checkCanSendReminders(user.id)

    if (reminderCheck.allowed) {
      // Планируем 4 напоминания
      const stages = [
      ...
      const reminderRows = stages.map(s => ({
        ...
        status: 'pending', recipient_email: clientEmail,
      }))
    ...
    await admin.from('activity_logs').insert({
      user_id: user.id, invoice_id: invoiceId,
      action: 'Invoice sent', type: 'invoice_sent',
      message: reminderCheck.allowed
        ? `Invoice emailed to ${clientEmail}. Reminder sequence scheduled.`
        : `Invoice emailed to ${clientEmail}. Upgrade to Pro for auto-reminders.`,
    })

    return NextResponse.json({
      ok: true,
      remindersScheduled: reminderCheck.allowed,
      upgradeMessage: reminderCheck.allowed ? null : reminderCheck.reason,
    })
  } catch (err: any) {
